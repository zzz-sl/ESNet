...
def train_main(
    ...
    if test_only:
        checkpoint_tmp = torch.load('/home/nbuser/SL/ESNet/checkpoints/checkpoint_ehull_500.pt')
        ...
        with torch.no_grad():
            for dat in test_loader:
                g, lg, _, kge, target = dat
                out_data = net([g.to(device), lg.to(device), kge.to(device), _.to(device)])
